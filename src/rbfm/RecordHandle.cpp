...
namespace PeterDB {
    ...
    RC RecordHandle::rawDataToRecordByte(char* rawData, const std::vector<Attribute> &recordDescriptor, char* recordByte, short & recordLen){
        // 1. write attrNum
        short attrNum = recordDescriptor.size();
        memcpy(recordByte, &attrNum, sizeof(attrNum));
        // 2. calculate offset
        short nullFlagLenInByte = ceil(attrNum / 8.0);
        short dirOffset = sizeof(attrNum);
        ...
        for(short i = 0; i < attrNum; i++, dirPos += sizeof(short)) {
            ...
            switch(recordDescriptor[i].type){
                ...
                case TypeVarChar:
                    ...
                    //std::cout << "size of string:"  << strLen << std::endl;
    ...
    RC RecordHandle::recordByteToRawData(char *recordByte, const short recordLen, const std::vector<Attribute> &recordDescriptor, char *rawData) {
        short attrNum = recordDescriptor.size();
        // 1. get null flags
        short nullFlagLenInByte = ceil(attrNum / 8.0);
            ...
            if (curr > 0){
                ...
                switch (recordDescriptor[i].type) {
                    case TypeInt:
                        ...
                        valPos += sizeof(int);
                        std::cout << "@ recordByte:" << recordDescriptor[i].name << ": " << *((int *)val1) << std::endl;
                        break;
                    case TypeReal:
                        char val2[4];
                        memcpy(val2, recordByte + valPos, sizeof(float));
                        valPos += sizeof(float);
                        std::cout << "@ recordByte:" << recordDescriptor[i].name << ": " << *((float *)val2) << std::endl;
                        break;
                    case TypeVarChar:
                        // 1. write varchar length
                        int strLen;
